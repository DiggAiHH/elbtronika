...
async function main() {
  const iterations = Number(process.argv.find((a) => a.startsWith("--iterations="))?.split("=")[1] ?? 3);
  const reportPath = process.argv.find((a) => a.startsWith("--report="))?.split("=")[1] ?? "./benchmark-report.html";
  ...
  console.log(`   • Tasks: ${report.summary.totalTasks} × ${iterations} iterations = ${report.summary.totalRuns} runs`);
